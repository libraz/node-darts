...
describe('replaceWords', () => {
  ...
  describe('Dictionary', () => {
    let dict1: ReturnType<typeof buildDictionary>;
    let dict2: ReturnType<typeof buildDictionary>;
    ...
    it('should handle overlapping words correctly', () => {
      ...
      const overlapText = 'app apple pineapple';
      const result = overlapDict.replaceWords(overlapText, (word: string) => `[${word}]`);
      expect(result).toBe('[app] [apple] [pineapple]');
      overlapDict.dispose();
    });

    it('should not replace anything if no matches found', () => {
      const noMatchText = 'I like grapes';
      const result = dict1.replaceWords(noMatchText, (word: string) => `<<${word}>>`);
      expect(result).toBe('I like grapes');
    });
  });

  describe('TextDarts', () => {
    let td: ReturnType<typeof TextDarts.build>;
