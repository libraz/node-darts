/**
 * This test file is for testing the fallback mechanism in src/index.esm.ts
...
const mockEsmRequire = jest.fn((id: string) => {
  if (id.includes('node_darts.node')) {
    return mockEsmNativeModule;
  }
...
describe('ESM Module Fallback Mechanism', () => {
  beforeEach(() => {
    ...
    // Setup the global.require mock
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (global as any).require = mockEsmRequire;
  });
  ...
  it('should use fallback mechanism when bindings fails on Windows CI', () => {
    ...
    const { dartsNative } = require('../src/index.esm');
  ...
  it('should handle require errors in fallback mechanism', () => {
    ...
    mockEsmRequire.mockImplementation((id: string) => {
      if (id.includes('node_darts.node')) {
