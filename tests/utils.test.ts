...
describe('Utils', () => {
  ...
  describe('validateFilePath', () => {
    ...
    it('should throw FileNotFoundError for non-existent files', () => {
      ...
      expect(() => validateFilePath(nonExistentFilePath)).toThrow(
        `File not found: ${nonExistentFilePath}`
      );
    });
  });
  ...
  describe('ensureDirectoryExists', () => {
    it('should create directory if it does not exist', () => {
      ...
      const newTestFilePath = path.join(newDirPath, 'test.txt');

      // Directory should not exist yet
      expect(fs.existsSync(newDirPath)).toBe(false);

      // Ensure directory exists
      ensureDirectoryExists(newTestFilePath);

      // Directory should now exist
      expect(fs.existsSync(path.dirname(newTestFilePath))).toBe(true);
    });
